...
import Board, { isPiece } from "./board";
import {EMPTY, TARGET, NORMAL_RED, NORMAL_BLACK} from "./board";
import {COLOR_FLAG, CROWN_FLAG, TARGET_FLAG} from "./board";
import {RED_PLAYER, BLACK_PLAYER} from "./board";
import {isRed, isBlack, isOpponent} from "./board";
...
class LocalMultiplayer extends React.Component {
  constructor(props) {
    super(props);
    var states = [];
  ...
  listMoveTargets(row, column) {
    let thisPiece = this.getSquareDisplay(row, column);
    if ((thisPiece & CROWN_FLAG) !== 0) {
      let targetList = [];
      for (let dy = -1; dy <= 1; dy += 2) {
        for (let dx = -1; dx <= 1; dx += 2) {
          let pos = [row+dy, column+dx];
          while (this.isInRange(...pos) && !isPiece(this.getSquareDisplay(...pos))) {
            const posCopy = [...pos];
      ...
      return this.listMoveNeighbors(row, column, isBlack(thisPiece)? -1 : 1)
        .filter(n => !isPiece(this.getSquareDisplay(...n)));
    }
  }
  ...
  resetProposedMoves() {
    ...
    for (var row = 0; row < BOARD_SIZE; ++row) {
      for (var column = 0; column < BOARD_SIZE; ++column) {
        if ((this.getSquareDisplay(row, column) & TARGET_FLAG) !== 0) {
  ...
  recurseJumpTree(row, column, jumped, thisPiece) {
    ...
    this.listHitNeighbors(row, column).filter(e => {
      if (isPiece(this.getSquareDisplay(...e.destination)))
        return false;
      if (jumped[e.victim[0]][e.victim[1]])
        return false;
      return isOpponent(thisPiece, this.getSquareDisplay(...e.victim));
    }).forEach(e => {
      ...
      for (let dx = -1; dx <= 1; dx += 2) {
        ...
        if ((thisPiece & CROWN_FLAG) !== 0) {
          while (this.isInRange(victimPos[0]+dy, victimPos[1]+dx)
              && !isPiece(this.getSquareDisplay(...victimPos))) {
            victimPos[0] += dy;
            victimPos[1] += dx;
          }
        }
        ...
        if (isPiece(this.getSquareDisplay(...destPos)))
          continue; // can't jump to here
        if (!isOpponent(thisPiece, this.getSquareDisplay(...victimPos)))
  ...
  calculateHitsForSide(player) {
    ...
    for (let row = 0; row < BOARD_SIZE; row++) {
      for (let column = 0; column < BOARD_SIZE; column++) {
        if (!isOwnPiece(this.getSquareDisplay(row, column), player)) {
          continue;
        }
  ...
  canCurrentPlayerMove() {
    ...
    for (let i = 0; i < BOARD_SIZE; ++i) {
      for (let j = 0; j < BOARD_SIZE; ++j) {
        let thisPiece = this.getSquareDisplay(i, j);
